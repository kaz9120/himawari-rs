...
**手順を書いたら実行して確かめる**。2026-08-14の点検で、そのとおり実行しても
動かない手順が4件見つかった。存在しないビルドfeature、2世代前のネットを指す
取得コマンド、サブコマンドの足りないスクリプト、ファイル名違いのリンク20件で
ある。実装が進んでも文書が追わないと、読み手を誤った場所へ連れて行く。
数値を書くときも同じで、出典（ネットのメタデータ、ログ、ADR）に当たる。

**文書はtextlintを通してからPRを出す**（[ADR-0178](docs/adr/0178-textlint-gate.md)）。
一文の長さ・読点の数・助詞の重複・冗長表現・誇張表現をCIが見る。

```
npm run lint        # docs/ と *.md と .claude/ を検査する
npm run lint:fix    # 自動で直せるものだけ直す
```
...
**「まだ無い」と判断する前に、コードで確かめる**。2026-08-14〜15に同じ型の
誤りが3回続いた。

- 棄却された参照追従5件をまとめ直すADRを起草したが、5件とも
  [ADR-0109](docs/adr/0109-reference-parity.md)の群で導入済みだった
  （[ADR-0160](docs/adr/0160-revisit-rejected-under-better-eval.md)）
- 文書が参照するスクリプト4本を「消えている」と報告したが、不採用案の記述と
  移行済みの過去形の記述で、現行手順は壊れていなかった
- SPRTの走行を分割して合算する仕組みを「作る」と提案したが、
  [ADR-0087](docs/adr/0087-sprt-resume.md)が `--resume` として実装済みだった

**棄却の記録も、文書の記述も、grepの件数も、現在の実装ではない**。無いことを
主張するときだけ、実装を読む手間を惜しまない。あることの確認は間違えても
すぐ気づくが、無いことの誤りは作業を丸ごと無駄にする。
...
**valid lossを足切りに使わない**（[ADR-0158](docs/adr/0158-mirror-factorizer.md)）。
...
- 本文に入れ子の半角括弧を書かない。release-pleaseのパーサが解析に失敗し、
  **その回のリリースが黙って止まる**。1件でも失敗すると全コミットを捨てて
  ...
  遅れる。2026-08-17に `(clip(a)*clip(b)+64)` で起きた。コードを示すなら
  バッククォートで囲むか、文で書く
...
まだ移していない操作は直接叩く。`build-shapes.sh`・`train-shapes.sh`・
`floodgate-cycle.sh`・`release-book.sh`・`league`・`profile` である。
