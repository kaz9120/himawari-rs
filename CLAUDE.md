...
**棄却したADRを掘り起こすときは、対象がまだ入っていないかコードで確かめる。**
2026-08-14に、棄却された参照追従5件をまとめ直すADRを起草したが、5件とも
[ADR-0109](docs/adr/0109-reference-parity.md)の群で導入済みだった
（[ADR-0160](docs/adr/0160-revisit-rejected-under-better-eval.md)）。棄却の
記録は当時の判断であって、現在の実装ではない。棄却から実装までの間に、
別の群が同じ場所を通ることがある。
