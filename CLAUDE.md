...
- 既定の上限は10,000ペアで、判定に至らなければ「見送り」として記録し
  ...
  過去の走行はすべてこの中で決着している。H1を採択しないので誤採択率は
  増えない。見送りは棄却とは別の状態で、後で群にして測り直せる
- 落ちた走行は[ADR-0087](docs/adr/0087-sprt-resume.md)の `--resume` で続ける。
...
書き切ってから走らせる（2026-08-29オーナー指示）。判定を見てから読み方を
決めない。

1. 仮説を1文で書く
2. 動かす変数は1つにする
3. 固定するものを明示する。データ量・ステップ数・valid集合・乱数の
   ような暗黙の変数が、介入に連動して動いていないか点検する
4. 各結果が何を意味するかを事前登録する

介入が第二の変数を連動させるなら、統制群を設計してから走らせる。仮説の
根拠になる測定は、走り出す前に解像度を尽くす。粗い測定で起動して精密
