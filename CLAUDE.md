...
**成果は長い時間軸で積む**（2026-08-29オーナー指示）。セッションは
タスクごとに区切られるが、求められているのはセッションをまたいだ成果で
ある。判断と測定をADRへ残すのはそのためで、次のセッションの自分が読者に
なる。学習やSPRTの待ち時間は空き時間ではない。floodgateの回収と分析
（[ADR-0152](docs/adr/0152-floodgate-cycle.md)・[ADR-0191](docs/adr/0191-blindspot-benchmark.md)）、
補助ツールの整備、文書の手入れ、次の実験の設計に充てる。着手の速さより
1件ごとの丁寧さを優先する。
...
**細かいタスクはGitHub Issueで回す**（2026-08-30オーナー指示）。小さな
気になる点（既定値の食い違い、表示の粗、重複コード、CIの遅さ）は見つけた
その場で `micro` ラベルのIssueに切り、待ち時間に消化する。作成と消化を
ハイペースで回し、ロードマップの実験とは分けて管理する。運用の定型
（資源の住み分け・等価性の検証の型・委任）はmicro-improvementsスキルが
持つ。

この前提を守らせる仕組みは `.claude/settings.json` にある
（[ADR-0181](docs/adr/0181-agent-surface.md)）。読むだけの `hmwr` は確認なしで
通る。force push・履歴の書き換え・`Cargo.toml` の編集は、実行前に確認を求められる。
mainへの直接pushは拒否される。`hmwr` が呼び出すツールの直接実行も拒否される。
セッションの終わりに `cargo fmt` とコミット本文の括弧を検査する。
...
棋力はfloodgateの対局者ページ（URLは `hmwr/tools/floodgate.py`）が一次情報になる。
...
- **対立仮説は着手時に決め、実行後に変えない**（[ADR-0163](docs/adr/0163-sprt-hypothesis-choice.md)）。
...
- 既定の上限は10,000ペアで、判定に至らなければ「見送り」として記録し
  mainへ入れない（[ADR-0217](docs/adr/0217-sprt-pass-cap.md)）。判定済みの
  過去の実行はすべてこの中で決着している。H1を採択しないので誤採択率は
  増えない。見送りは棄却とは別の状態で、後で群にして測り直せる
- 落ちた実行は[ADR-0087](docs/adr/0087-sprt-resume.md)の `--resume` で続ける。
  上限に達していないなら、条件を変えずに局数を積む
- 対立仮説は効きの見込みで選ぶ。+10以上を見込む変更は着手時にelo1=10と
  宣言すれば、4分の1の局数で決まる

既定条件で測れない変更がある。時間管理は参照実装の既定値が実戦の持ち時間
（floodgateの300+10）を前提にしており、10+0.1では最小思考時間が配分を支配する。条件を
...
他エンジンから移すときは、その値の前提となる仕組みまで移す。確かめる
のは3つある。係数のスケールが本エンジンで成り立つか、想定する発動率が同じか、
その値が前提にしている別の仕組みがないか。3つ目を落として失敗した例が2件ある
...
調整環境で最適化されている。本エンジンには独自に調整済みの状態があり、参照の想定値へ
寄せるほど弱くなった例が2件ある。singular率の較正は−81.2 Elo
（[ADR-0141](docs/adr/0141-singular-rate-calibration.md)）、参照にない明らかに不利な処理の除去は
...
教師データの分布を変える実験では、評価基準も一緒に動く
（[ADR-0136](docs/adr/0136-quiet-teacher-positions.md)）。静止化した教師で学習した
ネットは、非静止の検証集合で0.0285悪い値を出しながら、対局では
+20.3 Elo [+9.4, +31.2] で勝った。検証集合を学習データと同じ条件へ揃えてから
読む（`train.py --eval-only` で、静止化した検証集合に替えて測り直せる）。**採否は対局で決める**。

## 実験の規律

学習・SPRT・SPSAのように時間を消費する実験は、起動前に次の4点をADRへ
書き切ってから走らせる（2026-08-29オーナー指示）。判定を見てから解釈を
決めない。

1. 仮説を1文で書く
2. 動かす変数は1つにする
3. 固定するものを明示する。データ量・ステップ数・検証集合・乱数の
   ような暗黙の変数が、介入に連動して動いていないか点検する
4. 各結果が何を意味するかを事前登録する

介入が第二の変数を連動させるなら、統制群を設計してから走らせる。仮説の
根拠になる測定は、走り出す前に必要な精度まで詰めておく。粗い測定で起動して精密
測定を後追いさせない（[ADR-0190](docs/adr/0190-selfplay-decided-thinning.md)の
交絡の記録が反例）。
...
チェーンごとにずれた。`hmwr` がラップする操作の直接実行はhookが止める
...
| `data/logs/` | hmwrの実行ログ |
| `data/queue/` | 実験のステップの完了マーカー（ADR-0209） |
| `data/status/` | 長く走るコマンドの状態ファイル（ADR-0220） |
