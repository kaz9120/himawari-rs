...
specのステップを上から順に走らせ、終わるたびに完了マーカーを書く。途中で止まっても、
...
def add_parser(sub: argparse._SubParsersAction) -> None:
    ...
    t = ss.add_parser(
        ...
        description="対局は結果ファイル、学習は学習の記録、データは完了マーカーから読む。"
    ...
    t = ss.add_parser(
        "reset",
        help="完了マーカーを消し、最初から走り直せるようにする",
        description="消すのはステップの完了マーカーだけで、成果物には触らない。"
        "各コマンドの完了マーカーが残っていれば、走り直しても作り直しは起きない。",
    )
    t.add_argument("name", help="specの名前")
...
def _is_done(name: str, step: spec.Step) -> bool:
    """完了マーカーがあればTrue。マーカーと今のコマンドが違えば、specが途中で変わっている。"""
...
def _run_steps(args: argparse.Namespace, s: spec.Spec, log: Path, beat) -> int:
    for i, step in enumerate(s.steps, 1):
        ...
        if args.dry_run:
            code = _dry(step)
            if code != proc.OK:
                raise proc.Fail(f"dry-runが通らない: {s.name}/{step.id}", code)
            continue
...
def _dry(step: spec.Step) -> int:
    """ステップをdry-runで通す。引数の誤りはSystemExitで返る。"""
...
def reset(args: argparse.Namespace) -> int:
    ...
    for mark in marks:
        if args.dry_run:
            ...
            mark.unlink()
    if not args.dry_run:
        print(f"完了マーカーを{len(marks)}個消した: {args.name}")
    return proc.OK
    ...
    for name in targets:
        try:
            ...
            for step in s.steps:
                code = _dry(step)
                if code != proc.OK:
                    raise spec.Invalid(f"{name}/{step.id}: dry-runが通らない（終了コード {code}）")
        except proc.Fail as e:
