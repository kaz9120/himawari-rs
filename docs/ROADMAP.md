...
計測の詳細は[RESULTS.md](RESULTS.md)を参照。
...
| P6 | 学習基盤の完成 | データ量に対してvalid lossとEloが単調改善、ハイパラ固定 | 完了（v0.7.0） |
| P7 | ネットワーク構造の探索・決定 | 構造をフリーズし、本格学習に移行 | 完了（v0.7.0）。純粋HalfKP 256x2-32-32で確定 |
| P8 | 本格学習 + 探索改善 + データ拡大 | floodgate参戦可能な棋力 | 進行中（19.9億局面で+243.8 Elo。探索改善も継続） |
