# ロードマップと現在地

このプロジェクトの「やりたいこと・状況」はすべてこの文書で管理する。
GitHub Issuesは使わない。設計判断は [ADR索引](adr/README.md)、
未起草の設計論点はADR索引のバックログにある。
フェーズ管理は[ADR-0068](adr/0068-sprt-driven-versioning.md)で終え、
現在地・現行の最強構成・残作業の3節で運用する。
計測・検証の結果は [RESULTS.md](RESULTS.md)、改善アイデアは
[下の候補](#候補)、教師データの所在は [DATASETS.md](DATASETS.md) を参照。
更新タイミングは、フェーズの出入り・残作業の増減があったとき。

## 現在地

- バージョン: 0.25.0（2026-08-01時点）
- **参照実装（やねうら王）への追従キャンペーンを一巡させたところ**

2026-07-30に方針を切り替えた。単発の改善を積むのをやめ、機能差分を群単位で
まとめて埋める（[ADR-0109](adr/0109-reference-parity.md)）。あわせてライセンスを
GPLv3へ揃え、逐語的な移植を可能にした（[ADR-0108](adr/0108-license-gplv3.md)）。

**11群のうち10群が完了し、単純加算で+517.5 Eloを積んだ。** 内訳は下の
「参照実装への追従キャンペーン」の表にある。方針転換の直前3日は採択2件
（+17.7と+45.6）だったので、桁が変わったことになる。

この方針が効いた理由は、棄却の解釈を変えたことにある。**移植元は周辺の仕組みと
組で成立しており、1本だけ移すと支えを失った側が損をする。** 過去の棄却5件は
いずれも実装も測定も正しく、欠けていたのは組み合わせだった。

- [ADR-0084](adr/0084-lmr-cutnode.md)（LMRのcutNode項、-1.1）は12項のうち強める
  1本だけを足していた。12項を揃えたG2が+124.0
- [ADR-0097](adr/0097-capture-history.md)（capture history、+4.0）は振り分け閾値と
  枝刈りマージンへ繋がっていなかった。繋いだG1が+88.5
- [ADR-0104](adr/0104-ponderhit-time-accounting.md)・[0106](adr/0106-ponderhit-continue.md)・
  [0107](adr/0107-ponder-optimum-bonus.md)（ponder、-117.8と-54.3と棄却）は4本の
  支えのうち1本ずつだった。4本揃えたG8が+19.3
- [ADR-0075](adr/0075-razoring-margin.md)・[0096](adr/0096-rfp-margin.md)・
  [0052](adr/0052-nmp-dynamic.md)（razoring・RFP・NMP）はimprovingの定義を触らずに
  数値だけを寄せていた。定義ごと揃えたG4が+41.1

**棄却の記録は「効かない」ではなく「その形では効かない」と読む。**

参照実装の値が本エンジンで成立しなかった例も2件出た。G5のsingular率（設計点1割に
対し本エンジンは43.5%）と、G7の配分の分母（move horizon方式が2度とも通らない）で
ある。どちらも該当部分だけを据え置き、残りを採った。

### 評価関数と学習基盤

評価関数の構造は純粋HalfKP 256x2-32-32で確定した。19.9億局面の学習に
factorizerを併せ、対halfkp_370Mで**+243.8 Elo**、factorizer分で
**+28.1 Elo**を得た（[ADR-0066](adr/0066-halfkp-factorizer.md)）。
FT次元を512へ広げる案は、評価精度で上回るもののNPSが0.65倍に落ちる
代償が大きく不採択になった（[ADR-0067](adr/0067-ft-dimension-512.md)）。

学習基盤は41,000から449,000 samples/sへ改善し、19.9億局面の1エポックが
2時間で回るようになった（[ADR-0064](adr/0064-dense-ft-gradient-mps.md)・
[ADR-0065](adr/0065-large-scale-dataloader.md)）。

### 追従キャンペーン以前の探索改善

2026-07-21〜25のキャンペーンで8件を採択し、単純加算で約+544 Eloを積んだ
（correction history +44.6、continuation history +20.7、eval hash +54.1、
singular +12.6、ProbCut +44.2、qsearch TT +113.6、razoring +184.8、
思考時間の難易度スケール +69.3）。

2026-07-28〜29に選定基準を[ADR-0089](adr/0089-improvement-criteria.md)へ
起こし、速度・ノード効率・終盤の正確さの3軸で候補を評価する運用にした。
この基準は単発の着手判断としては残るが、**追従の群には適用しない**
（[ADR-0109](adr/0109-reference-parity.md)）。群に求めるのは「単独で効く仮説」では
なく依存の閉包である。

同時期に実装の穴を5件埋めた。MultiPV出力の降順が崩れる不具合、詰みを
読み切っても深さ127まで回る挙動（[ADR-0088](adr/0088-mate-early-stop.md)）、
`seldepth` と `currmove` の欠落（[ADR-0086](adr/0086-search-observability.md)）、
aspirationのfail high/lowを報告していない件
（[ADR-0092](adr/0092-aspiration-bound-info.md)）、SEEが駒打ちを解いて
いない件（[ADR-0091](adr/0091-see-drop.md)）である。測定側も
[ADR-0087](adr/0087-sprt-resume.md)でSPRTに中断耐性を入れ、
`scripts/verify-feature.sh` で機能検証の局面と深さを固定した。

2026-07-24〜25にfloodgateへ初参戦した（30局19勝11敗、レート3186）。
負けはすべて相手レート3121以上で、実力の境界が3100〜3250にある。
負け11局はすべて長手数（105手以上）で、終盤10手の消費が7〜9秒に
落ちていた。持ち時間の枯渇であり、時間配分の設計が原因
（[ADR-0059](adr/0059-easy-move-scaling.md)で対処）。

当面の目標は2027年5月の世界コンピュータ将棋選手権への参加で、
参加時のビルドを1.0.0とする（[ADR-0068](adr/0068-sprt-driven-versioning.md)）。
そこまでは棋力向上をSPRTで1件ずつ積み、採択のたびにMINORを上げる。
実力の壁はfloodgateのレート3100〜3250にあり、まずここを越える。

計測の詳細は[RESULTS.md](RESULTS.md)を参照。開発の進め方は
[ADR-0070](adr/0070-pr-based-workflow.md)（PRベース、種別ごとのマージ条件）。

### 現行の最強構成

| 項目 | 実体 |
|---|---|
| エンジン | FT256ビルド（`data/bin/himawari-ft256`） |
| 評価関数 | `data/nets/halfkp_1900M_fact.hmwr.best`（valid loss 0.49513） |

FT512は評価精度で上回る（valid loss 0.49374、train loss 0.48338）が、
NPSが0.65倍に落ちる代償を取り返せず、SPRTで-72.8 Elo（968局、H0採択）
だった（[ADR-0067](adr/0067-ft-dimension-512.md)）。比較を続けられるよう、
512のバイナリ（`data/bin/himawari-ft512`）とネット
（`data/nets/halfkp_1900M_ft512.hmwr.best`）も残してある。

## 次の方向

**候補の一覧は[下の候補](#候補)にある。** ここには「今どこにいて、次にどちらへ
向かうか」だけを書く。着手を決めた案はADRを起こしてROADMAPの候補から消す。

### 1. floodgateで実戦を確認する

追従で+517.5を積んだ状態がレートでどこに現れるかを見る。2026-07-24〜25の初参戦は
30局でレート3186、実力の境界が3100〜3250にあった。

**追従で入れた機能のうち4つは既定条件のSPRTで測れていない。** 定跡のponder手
（`BookFile` の指定が要る）、best thread voting（`Threads=4`）、Lazy SMPの窓幅
多様化（同上）、時間管理の既定値（`MinimumThinkingTime=2000` は300+10向けで、
10+0.1では床が配分を支配する）である。floodgateは300+10なので、参照実装の既定値が
本来想定する条件に近い。**実戦が次の測定の場になる。**

### 2. 評価関数と探索を結合する

多ヘッド出力（WDL・進行度・安定度）を学習し、時間管理・枝刈りの強度・contemptへ
供給する（[ADR-0120](adr/0120-after-parity.md)）。

追従で探索は参照実装と同じ構造になった。**同等より先へ行くには別の軸が要る。**
本エンジンの独自資産は評価関数を自前で学習していることで、参照実装が持たない
領域である。あわせて[ADR-0089](adr/0089-improvement-criteria.md)の3軸へ第4の軸
「判断材料の質」を足す。

工程が長い。学習器・ネット・探索の3つが揃って初めて測れるので、1群1SPRTのような
短い周期にならない。

### 制約

- **持将棋のルールが2028年の選手権から27点法→24点法へ変わる見込み**
  （2026-07-20オーナー共有。詳細規定は公式発表を正とする）。影響するのは3か所で、
  宣言勝ちの判定（[ADR-0030](adr/0030-nyugyoku-declaration.md)は27点法/CSA実装）、
  自前gensfenの勝敗ラベル、selfplayの入玉局面裁定である。公開データセット
  （hao_depth9等）は27点法時代の生成物だが、入玉決着は少数なので教師あり学習への
  影響は無視できる
- 教師データの残り: hao_depth9のstart_time=1695872823（127ファイル、約10億局面）が
  未取得。29.9億まで増やせるが、容量律速のうちは伸びが逓減する
- 持ち越し: x86_64でのperft・ベンチ再計測（環境入手待ち）

## 候補

未着手の案を1案1行で置く。**着手を決めたらADRを起こしてここから消す。**
完了・棄却した案も残さない（経緯と結果はADRが持つ）。

検証手段の略記: SPRT=対局ゲート、loss=検証損失、NPS=速度計測。
他の案を前提とする案は、メモ欄に依存先を書く。表の並び順は優先度を表さない。

単発の候補は[ADR-0089](adr/0089-improvement-criteria.md)の3軸（速度・ノード効率・
終盤の正確さ）で選ぶ。どの軸に乗るかと、単独で効く仮説を書けない候補は着手しない。
**参照実装に対応物がある候補はこの判断を使わず、群単位で移す**
（[ADR-0109](adr/0109-reference-parity.md)）。

### 探索

過去にSPRTで棄却した案のうち、P3期（駒割評価）の測定は条件が違う。
singular extensionがP3の-16.0からP8で+12.6に反転した先例があり、
NNUE後の再測定に意味がある。

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| see_geの打ち対応 | 現状は打つ手のSEEを常に0とし、打った駒がその場で取られる筋を見ていない。将棋は打つ手の比率が高く、ADR-0090のSEE枝刈りをすり抜ける | SPRT |
| singularの多段化（保留） | [ADR-0114](adr/0114-g5-singular.md)で保留。double/tripleを入れると群全体が-99.0になった。本エンジンのsingular率が43.5%で参照実装の設計点1割から4.4倍外れており、+3まで積む機会が多すぎる。再訪の条件はsingular率を設計点へ寄せる方法が見つかったとき | SPRT |
| mate_1plyの通常探索への組み込み | [ADR-0115](adr/0115-g6-qsearch.md)（G6）で静止探索へ入れ、本体を指さない方式へ書き換えて6.5倍にした。[ADR-0093](adr/0093-mate1ply-in-search.md)が-57.5で棄却した理由（呼び出しコスト）は消えている。原典はS:2896-2960 | SPRT |
| mate_1plyの見逃し率改善 | 現状の見逃しは近傍のみで10.4%、遠隔駒ありで16.0%。原典は遠打ちの合駒不能判定や王手クラスをより広く扱う。合駒の判定も「相手に持ち駒があれば遮れる」という安全側の近似のまま | SPRT |
| df-pn詰み探索 | 長手数詰み。終盤力・宣言勝ち周りの取りこぼし対策 | SPRT |
| SEE駒価値のNNUE時代適合 | 枝刈り閾値系の駒価値を再調整 | SPRT |

### 探索: 並列・置換表

既定が `Threads=1` なので、追従で入れた並列まわりの2件は未測定である。
測るには1局あたりのコア数が増え、並列度を落とす必要がある。

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| best thread votingの測定 | [ADR-0119](adr/0119-g10-book.md)（G10）で実装済み。`Threads=4` で測る | SPRT |
| Lazy SMPの窓幅多様化の測定 | [ADR-0118](adr/0118-g9-aspiration.md)（G9）で `delta` に `threadIdx%8` を入れた。Threads=1では効かない。[ADR-0031](adr/0031-lazy-smp.md)の「多様化しない案B」を見直す材料になる | SPRT |
| historyのスレッド共有 | 参照実装はcorrection historyとpawn historyをNUMA単位で共有する。現状は全てスレッドローカル | SPRT |
| TTクラスタの手番分割 | インデックスの最下位ビットを手番で置換し、手番違いの衝突を消す | SPRT |

### ネットワーク構造

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| output bucket（駒数で層分岐） | 局面フェーズ別の専用ヘッド。SF系で効果大 | SPRT |
| PSQT直結パス（material head） | FTから評価へのskip。序盤の材料感を安定化 | loss+SPRT |
| 玉ライン特徴（8方向最近接駒） | ADR-0044で検証。valid lossで利き塔の87%を低コストで実現。差分計算可能 | loss+SPRT |
| 手番特徴の明示化 | HalfKPは手番を視点でしか持たない。tempo項の学習 | loss |
| 多ヘッド出力（WDL・進行度・安定度） | 時間管理・枝刈り強度・contemptへの供給源（ADRバックログ済み） | SPRT |
| 量子化の再検討（FT i8等） | メモリ帯域半減でNPS向上。ADR-0067でFT512がNPS律速と分かったため優先度が上がった。512の容量を活かす本命 | NPS+SPRT |

### 評価: 入玉局面

2026-07-29のfloodgate対局を入玉絡みと読んだのは誤りで、この節の前提は
崩れた（RESULTS.mdの訂正を参照）。入玉局面の評価精度が未検証なのは事実
なので節だけ残すが、優先度は下げる。

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| 入玉局面の評価精度を測る | 教師データ（hao_depth9）は他エンジン由来で入玉が薄い。ただし実戦で問題が出た証拠はまだない | 実測 |
| 評価値のクランプ | 現状は±31871で実質無制限。外挿された極端な値がそのまま出る | 要検討 |

### 学習

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| 教師局面のqsearch静止化 | データセットREADMEも指摘。静止局面で学習し評価の整合を取る | loss+SPRT |
| λのply依存化 | 序盤はresult重視、終盤はscore重視の混合比 | loss+SPRT |
| 詰みスコアの扱い | ±30000近傍のclamp/除外/専用ターゲット。現状8.5%を素通し | loss+SPRT |
| ミラーデータ拡張 | 左右反転で実質2倍。盤面対称性の担保にも | loss |
| EMA重み平均（SWA） | 終盤の重み振動を平均化。ほぼ無料で数Elo | SPRT |
| 継続学習の世代運用 | 前世代ネットからfinetune。lr小さめで積む | SPRT |
| 教師との指し手一致率の計測 | PSVのmoveフィールド活用。lossと別の健全性指標 | 指標 |
| 複数データセット混合 | hao系+水匠系等。分布の偏り緩和（ライセンス確認） | SPRT |
| 序中終盤のサンプリング重み | gamePly別の採択率調整 | loss+SPRT |
| 公開ネットウォームスタート対照 | ADR-0039の対照実験。互換ローダで初期化 | SPRT |
| ハイパラのSPRT運用 | lr・λ・epochsを1変更=1SPRTで積む | SPRT |

### 教師データ・基盤

| 案 | 狙い・メモ | 検証 |
|---|---|---|
| 学習チェックポイント | f32状態の保存・再開。長時間学習の中断耐性 | 動作 |
| gensfen自前実装 | 24点法裁定・開始局面ランダム化・乱択（バックログ済み） | 動作 |
| RL世代ループ | 自分の深い探索を教師に世代を積む（バックログ済み） | SPRT |
| 大規模対局基盤 | SPRTの並列度向上、複数マシン分散 | 運用 |
| floodgate参戦 | 実戦レーティングの定点観測。対人間系の弱点発見 | 運用 |
| NPS回帰のCI監視 | ベンチ局面のNPSをCIで記録し、退行を検知 | 運用 |
| 定跡の掘り下げ | ADR-0063の133局面を現行ネットで作り直す。選択方法と足切りは[ADR-0109](adr/0109-reference-parity.md)のG10で扱う | 運用 |

## 学んだこと

判断の根拠になっている経験則。詳細は各ADRにある。

**移植の単位は依存の閉包である。** 移植元の機能は周辺の仕組みと組で成立して
おり、1本だけ移すと支えを失った側が損をする。過去の棄却5件はいずれも実装も
測定も正しく、欠けていたのは組み合わせだった。**棄却の記録は「効かない」では
なく「その形では効かない」と読む**（[ADR-0109](adr/0109-reference-parity.md)）。

**その値が何に守られているかを見る。** 忠実に移して裏目に出た例が2件ある。
optimumがNetworkDelayを引かない設計は床（1880ms）の保護に、
bestMoveInstabilityはroot探索ごとの安定ソートに守られていた
（[ADR-0116](adr/0116-g7-timeman.md)・[ADR-0118](adr/0118-g9-aspiration.md)）。
他エンジンから移すときに確かめるのは3つ。係数のスケール、発動頻度の設計点、
その値を守っている別の仕組みである。

**参照実装の値が本エンジンで成立しないことがある。** 2件出た。singular率
（設計点1割に対し43.5%。[ADR-0114](adr/0114-g5-singular.md)）と時間配分の分母
（move horizon方式が2度とも通らない。[ADR-0116](adr/0116-g7-timeman.md)）である。
該当部分だけを据え置き、残りを採った。

**要約は必ず細部を落とす。** 作業指示書と原典の食い違いが9群すべてで出て、
合計30箇所以上、すべて原典が正しかった。移植は原典を読んで行う。手順は
[.claude/skills/porting-from-reference](../.claude/skills/porting-from-reference/SKILL.md)に
ある。

**差分を洗う作業はバグ発見の経路になる。** 本エンジン単独のバグが3件出た。
ttPvのTTへの書き戻し、lmrDepthの基準、除外手つき探索の返り値である。

**既定条件で測れない変更がある。** 時間管理は参照実装の既定値が実戦の持ち時間を
前提にしており、10+0.1では床が配分を支配する。条件を変えるか非劣性検定へ落とす
（[ADR-0116](adr/0116-g7-timeman.md)）。

**CIはローカルより遅い。** 時間に依存するテストを書かない。判定はマシンの速度に
依存しない量で行う（[ADR-0117](adr/0117-g8-ponder.md)のG8でCIが落ちた）。
