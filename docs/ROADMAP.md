...
SPRTを省いた。G1は762局で+88.5、G2は508局で+124.0、G3は718局で+95.2、
G4は1672局で+41.1、G5は1530局で+48.2を得てH1採択した。5群の単純加算で
+397.0になる。G7とG10はG0に依存しないため、G6以降と並行して進められる。

**G5は群全体では-99.0でH0だった。** 二分割して多段延長を除いた形が+48.2で
通っている。参照実装の値が本エンジンで成立しなかった最初の例で、成り立た
なかった前提はマージンのスケールではなくsingular率（発動頻度の設計点）
だった。参照実装は1割、本エンジンは43.5%である。
