...
- 定跡（[ADR-0063](adr/0063-book-loader-and-mini-book.md)）は実装・生成
  ...
  作り直しは実戦の結果を見てから判断する
- 時間管理の残り: 配分式そのものの再設計は
  [ADR-0102](adr/0102-move-horizon.md)で試して-107.2で棄却した。
  やねうら王のmove_horizon方式を移したが、一律に薄くしたため最善手が
  後から変わる局面からも時間を奪った。次は削ってよい局面の見分けを
  鋭くする方向へ進む。候補はIDEAS.mdの「探索: 時間管理」節にある
