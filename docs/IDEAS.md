...
枝刈りの危険度のグラデーションも同ADRにある。危険度の低いSEEベースの
枝刈り2件（quiet SEE・capture SEE）が未実装で、危険度の高いものを
増やす前にここを埋める。
...
| LMRの項を群として移植 | ADR-0076（ttPv、-43.8）とADR-0084（cutNode、-1.1）で、1項ずつ積む方針が2件続けて否定された。cutNode・ttPv・statScore・moveCount・correction値・ttCapture・cutoffCntをまとめて入れ、群として1回のSPRTで測る。配管はadr-0084-cutnodeブランチにある | SPRT |
| LMR再探索の深さ調整 | doDeeperSearch（value > bestValue+48）とdoShallowerSearch（value < bestValue+9）でnewDepthを±1する。本エンジンの再探索条件は d < new_depth だけで、強く削った結果を拾い直す仕組みが弱い。上の群移植より先に要るかもしれない | SPRT |
...
| 最小思考時間 | MinimumThinkingTime（やねうら王の既定は2000ms）。ADR-0102で「SPRT条件でスケール前提が成り立たない」と判断して見送った | SPRT |
| ponder有効時の思考時間1.25倍 | Stockfishの `optimumTime += optimumTime / 4`。ADR-0104では会計と組にして測って棄却したが、会計を入れない以上は単独で意味を持つ。[ADR-0107](adr/0107-ponder-optimum-bonus.md)で測る | SPRT |
...
| 案 | 狙い・メモ | 検証 |
|---|---|---|
| **窓幅を評価値の大きさで広げる** | 初期窓が `delta = 20` 固定。評価値2800では±0.7%しかなく、窓外れが1周6.35回まで増える（RESULTS.md）。StockfishもやねうらOも評価値に応じて広げる。ノード効率の軸（ADR-0089の軸2） | SPRT |
| fail low時のbetaの下げ方 | 現状は `beta = (alpha + beta) / 2` でbetaをalphaへ寄せる。直後のfail highと往復する要因になりうる | SPRT |
...
| 案 | 狙い・メモ | 検証 |
|---|---|---|
| 入玉局面の評価精度を測る | floodgateの2026-07-29の対局で、入玉含みの局面を+4723と評価して負けた（RESULTS.md）。教師データ（hao_depth9）は他エンジン由来で入玉が薄い。まず現行ネットの入玉局面での誤差を測る | 実測 |
| 入玉局面の教師データを足す | 上で誤差が大きければ、gensfenで入玉局面を生成して学習に混ぜる。持将棋は24点法（ADR索引のバックログ） | 学習+SPRT |
| 評価値のクランプ | 現状は±31871で実質無制限。外挿された極端な値がそのまま出る。他エンジンは評価の出力域を絞る | 要検討 |
