...
ponderは当初このADRに含めていたが、別ADRへ分離した（バックログ参照）。
相手番思考は持ち時間を実質的に増やす棋力機能である。ponderhit後の時間
配分や「ponder中に詰みを見つけて即bestmoveを返す2手指し」のような定番
バグへの防御を含めて、単独で設計すべきものになる。
...
- K=1が既存探索と同一ノード数になることをテストで固定する
  ...
  ことを検査する
- 棋力の検証は非劣性SPRT（K=1経路が等価であることの確認）

## Consequences

- RootMove化は探索の可読性を上げ、後続ADR（Lazy SMP・ponder）の
  土台になる
- MultiPVのK>1では探索効率が落ちる。それが正常である。検討モード専用で、
  対局時はK=1を前提とする
