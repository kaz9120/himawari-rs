...
3. **手順をスキルに固定する。** `.claude/skills/running-sprt/SKILL.md` が
   起動（build-pair→verify→sprt.sh）・監視・終了後（H1/H0/判定に至らず
   それぞれの記録と後処理）を持つ。エージェントはSPRTのたびにこれを使う
