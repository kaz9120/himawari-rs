# 0220: 開発環境を再設計する。状態の出力仕様・ポータル・コマンドの棚卸し・遠隔制御

- Status: accepted（2026-09-22、オーナーが4点を決めた。下の「決定」。4・5段目は[ADR-0221](0221-public-site.md)で取りやめた）
- Date: 2026-09-22
...
- **ログの行の形式がコマンドごとに違う**。進み具合を機械で読むには、
  コマンドごとに解釈が要る
- **実行中の対局を止めるサブコマンドがない**。ADR-0211の打ち切りはSIGTERMで行い、
...
`data/status/<領域>-<名前>.json` を更新する。60秒ごとと、開始・終了時に
書く。ログ（`data/logs/`）は人が読む用のまま残し、機械は状態ファイルだけを読む。
...
ラベル）、実行中の状態ファイル、直近の結果（`.result`、学習の記録）、資源（ディスクの
空き、launchdの稼働状況、GPUの使用）、開いているPR。`hmwr status`（JSONなし）は
...
```
crates/        Rust（エンジン・ツール）
hmwr/          Python CLI（状態の出力仕様の持ち主）
training/      学習器
experiments/   実験のspec
...
| 停止のサブコマンドがない | `match stop <名前>`、`exp stop <名前>` | 停止ファイルをペアの切れ目で見る。見送りとして結果に残す（#581） |
| `queue` の積む・外す・並べ替えが手作業 | `queue add <spec>`、`queue drop <番号>`、`queue move <番号> --before <番号>` | Issueの作成とラベル付けをラップする。一次情報はGitHubのまま |
| 進み具合はログだけ | 状態ファイル（上の1） | 機械が読める |
| `hmwr env` | `hmwr status` に吸収 | 見る場所を1つにする |

作り直しは、出力仕様（1）に合わせてサブコマンドを切り直す形で行う。既存のspecと
...
| 画面 | 中身 |
|---|---|
| 今 | 実行中の実験とステップ、進み具合と残り時間、対局のEloとLLR、学習のstepとvalid |
| キュー | 待ちの実験の並び。一時停止・再開・積む・外す・並べ替え・停止 |
| 結果 | 直近の結果（対局・学習）と、記録PRへのリンク |
| 資源 | ディスクの空き、launchdの稼働状況、GPUの使用、開いているPR |

制御はserverが `hmwr` のサブコマンドを呼ぶ。外からの入口はCloudflare Tunnelだけにする。
手前にCloudflare Accessを置き、serverもJWTを検証する。pr_inboxの
`docs/operations.md` と同じ対策である。常駐はlaunchd。
...
| 段 | 内容 | 目安 |
|---|---|---|
| 1 | 状態ファイルの形式と `hmwr status --json`。まずCLIだけで動かす | 2日 |
| 2 | 長く走るコマンド4つに状態ファイルを足す。ADR-0219の実行が最初の実物 | 1日 |
| 3 | `portal/` を作り、読むだけの画面（今・結果・資源）を出す | 3日 |
| 4 | launchdとTunnel・Accessで外から見る | 半日 |
| 5 | 停止・積む・外す・並べ替えのサブコマンドと、キューの画面 | 2日 |
...
1. ポータルの認証はCloudflare Accessだけにする。LANからAccessなしの入口は
   作らない
...
| 段 | PR | 内容 |
|---|---|---|
| 1 | #602 | 状態ファイルの形式、`hmwr status`、付け直しの状態ファイル |
| 2 | #606 | 学習・対局・実験・SPSAの状態ファイル。`hmwr clean` が30日で消す |
| 3 | #609 | `portal/` と読むだけの3画面（今・結果・資源） |
| 4・5 | 取りやめ | 外から見る役は公開ページ（[ADR-0221](0221-public-site.md)）へ移した。`portal/` も消した |

2段目の組み込みは、子プロセスの出力の行を読んで状態ファイルを更新する形にした。学習と
対局は進み具合を標準出力にしか出さないので、`proc.run` に行ごとのコールバックを
足した。子プロセスの側は変えていない。実験のステップは所要が数分から
数日まで揃わないので、残り時間を見積もらない。SPRTもいつ判定に至るか
分からないので、総量を持たない（指し切りのときだけ持つ）。

3段目では、2の「リポジトリの構成」から1点を変えた。ルートの `package.json` を
workspaceの親にせず、`portal/` を独立したBunのworkspaceにした。ルートは
textlint用にnpmで入れており、CIの文書のジョブは `npm ci` で動く。親にすると、
文書のlintのためにポータルの依存まで入れることになる。ポータルは `portal/` の
中で完結させ、CIには別のジョブを足した。

入口の守りも1点をpr_inboxから変えた。pr_inboxはAccessが未設定なら検証を
省く。こちらは未設定なら、localhost以外からの要求をすべて断る。決定1の
「Accessなしの入口を作らない」を、設定の漏れでも破らないためである。

3段目を開いたオーナーは、手元の運用画面より公開ページのほうが良いと
判断した（2026-09-24）。公開ページは読むだけで操作を持たないので、
TunnelとAccess（4段目）と遠隔操作（5段目）は要らない。`portal/` も消し、
このリポジトリはエンジンと補助のコードに絞る。詳細は[ADR-0221](0221-public-site.md)にある。

## Consequences

- `data/status/` が増え、`hmwr clean` の対象になる。状態ファイルは終了後も残し、
  30日で消す
- ポータルのためにBunとNodeの依存がリポジトリに入る。CIの所要が伸びる
- コマンドの統一で、スキルとADRの手順の記述を追従させる。別名を残す間は
  二重に見える
- 見直しのトリガーは、状態ファイルの形式で表せない状態が出たときである。そのときは
  `detail` を広げず、形式を改版する
