...
ただしそこでは「ponderで読んだ時間を予算に数える」変更と**組で**
測っている。時間を厳しく数える側と補正する側は対だ、という理由からだった。

そのponder時間の計上は-117.8で棄却され、[ADR-0106](0106-ponderhit-continue.md)で
前提（探索の継続）を足しても差し引きがほぼゼロだった。**ponder時間の計上を入れない
...
にもかかわらず配分はponderなしと同じである。ponderで得た探索の分だけ、
...
optimumの比がどちらも1前後で揃っている。ponderで得る探索の割合が
近いので、既定条件の結果は実戦にも当てはまると期待できる。
...
| 案 | 結果 |
|---|---|
| 計上だけ（[ADR-0104](0104-ponderhit-time-accounting.md)） | -117.8 |
