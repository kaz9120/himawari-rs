...
- perftは `crates/tools` のbinとして実装する（ADR-0002の構成）。
  ...
  `--slow` フラグで素朴実装に切り替えられるようにする
- テーブル駆動のテストデータは次の3群
  1. 平手初期局面: depth 1〜5（30 / 900 / 25,470 / 719,731 / 19,861,490）は
     CI（release）で確認する。depth 6（547,581,517）はローカル手動で確認する
  2. エッジケースSFEN集: 打ち歩詰め、pin、両王手、最大分岐局面
     （593手）、入玉形。公開されている値と照合し、値のない局面は
     素朴実装の結果を記録して回帰テスト化する
...
- perftのNPSをcriterionとは別に記録し、盤面実装の性能回帰の
  検出に使う（bulk countingの値で記録する）
