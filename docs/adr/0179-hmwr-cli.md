...
問題は**系統ごと、さらには1本ごとに流儀が違うこと**にある。使う側が覚える対象は
コマンドの数だけある。
...
- **サブコマンドを持つのは `fetch-dataset.sh` だけ**である。ほかは1本1機能で、
  ...
  `build-shapes.sh`）
- **dry-runがあるのは `release-*.sh` だけ**である。1時間かかる学習や、
  数日かかるSPRTにはdry-runの手段がない
...
連想配列が使えない。流儀を統一する層をshellで書くと、統一したい対象と
同じ落とし穴を踏む。
...
1. **オプションはフラグで渡す**。環境変数への変換はCLIが行う。
   `hmwr train x --data d.psv --lr 1e-4` と書けば `TRAIN_PEAK_LR=1e-4` へ変換する
2. **実験名を検証し、ログの置き場をCLIが決める**。`data/logs/<領域>-<名前>.log`
   になり、領域はサブコマンドから決まる。呼び出し側はリダイレクト先を書かない
3. **`--dry-run` ですべてのコマンドを事前に確認できる**。走るはずのコマンド列を
