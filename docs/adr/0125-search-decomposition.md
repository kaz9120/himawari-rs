...
結果である。**動いているが、次の変更を入れる土台としては読みにくい**。
...
参照実装の Step 番号がコメントで刻まれているので、切り出す境界は既に見えている。
...
追従で入れた機能を参照実装と突き合わせられなくなり、次の追従が難しくなる。

## Decision

案Bを採る。

**決め手は、切り出す境界がすでにコメントで示されていることである**。参照実装のStep番号に沿って
...
- 次の変更を入れる土台が読みやすくなる。1006行の関数へ機能を足すより、責務ごとの関数へ
  足すほうが影響範囲を見切りやすい
- 参照実装との対応は保たれる。Step番号のコメントは関数へ移すので、参照実装の行番号で照合できる
