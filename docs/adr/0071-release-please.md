...
ネットをリリースに含めない方針（ADR-0069）は変わらない。
...
worktreeが汚れる。無関係な差分が各PRへ混ざり、1コミット=単一の論理変更
...
## 事故の記録: リリースPRの自動マージとfeatの競合（2026-08-22）

タグがfeatの後ろへ打たれ、+32.0のfeatがCHANGELOGに載らないまま
「リリース済み」扱いになった。経緯は3段である。

1. fixのマージでrelease-pleaseがv0.41.1のリリースPRを作り、
   オーナーが自動マージを設定した
2. その直後にfeatがマージされた。featを取り込むrelease-pleaseの実行が、
   同時に進んだリリースPRの自動マージとブランチ更新で競合して失敗した
3. リリースPRのマージでタグがmain先頭（featより後ろ）へ打たれた。
   release-pleaseはタグ以前のコミットをリリース済みとみなすので、
   featは以後どの提案にも載らない

**リリースPRの自動マージは、直後の別マージと競合しうる**。リリースPRを
マージする（自動マージを設定する）前に、直近のrelease-please実行が
成功していて提案が最新かを見る。逆に、リリースPRが開いている間に
feat/fixをマージしたら、提案が更新されるのを待ってからリリースPRを
マージする。

復旧手順（今回実施）は次のとおり。

1. 該当のリリースとタグを削除する（オーナー確認のうえで行う）
2. リリースPRのマージコミットをrevertするPRを入れ、CHANGELOG・
   manifest・版を前のリリースへ戻す
3. release-pleaseが次のpushで正しい提案を作り直す（今回はv0.42.0として
   featとfixの両方が載った）

失敗した実行はActionsのログに「Error updating ref
heads/release-please--branches--main」と残る。コミット解析の失敗
（入れ子括弧）と違い、こちらは競合なので再実行では直らず、タグの位置が
問題になる。
