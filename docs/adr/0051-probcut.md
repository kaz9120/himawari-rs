...
発動条件（ムーブループ前、NMPの後）:
- non-PV、非王手、除外手つき探索中でない（ADR-0050で除外手を渡す仕組み）
- depth >= 5
