...
ガードで発動しない可能性がある。効果が出ない場合は `--tc 60+1` で
追試する。深さ下限を下げるのは定数チューニングにあたるので本ADRの
範囲外とする。

SPRTとは別に、読み抜けの有無を直接確認する。`bench` は深さ固定で
本変更の影響を受けないため、時間制限つきで既知の戦術局面を解かせ、
変更前後で最善手が変わらないことを確かめる。
