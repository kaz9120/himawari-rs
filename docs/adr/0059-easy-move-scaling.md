...
状態はカウンタ1個で済む。ただし伸ばす係数がない。評価が下落中でも、
最善手が変わっていなければ打ち切る。読み抜けが進行している局面を
すり抜ける。

### 案B: 縮める指標と伸ばす係数を積で合成する
...
等重みで入れて棄却になった教訓を適用したためである。これは誤った
適用だった。ADR-0055の2項は独立した調整項で、分離して測る意味が
あった。本ADRの係数は、伸ばす係数を含む1つの組である。分離して
SPRTを回すと、H0採択が「機能自体の否定」なのか「伸ばす係数の欠如による
事故」なのか切り分けられない。
...
ではなく、伸ばす係数が働いていることの確認になる。終盤は消費を9%減らし
ながら到達深さが1深くなった。合計17%の節約である。
