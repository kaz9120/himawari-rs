...
既存の公開ネットの読み込み互換（ADR-0034）を守るため、
...
- 出力: int32をFV_SCALE = 16で除算し、歩=90スケールの
  評価値に写像する
- 利き入力（ADR-0034）の線形層も同スキーム（重みint8、
  活性0..127）に従う。互換検証（利き入力ゼロ）の一致性を保つため

### SIMD実装

- まずスカラーの基準実装を書く。これがSIMD版の照合相手であり、
...
- `std::arch` のintrinsicsへの書き換え（unsafe、ADR-0004の適用対象）は、
  std::simdで性能が不足すると計測で示されたときだけ検討する

### 検証

- 「SIMD = スカラーの完全一致」をランダム局面の全数照合で
  固定する（P4出口条件）。飽和・丸めの経路もビット一致を要求する
