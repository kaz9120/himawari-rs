# 0117: ponder時間の計上・継続・予約を参照実装へ揃える（G8）
...
[ADR-0109](0109-reference-parity.md)による一連の追従の第9群。ponderまわりを
参照実装（やねうら王）へ揃える。
...
| ADR | 移したもの | 結果 |
|---|---|---|
| [0104](0104-ponderhit-time-accounting.md) | ponder中の時間を予算に計上する | -117.8 |
| [0106](0106-ponderhit-continue.md) | ponderhitで探索を継続する | -54.3 |
| [0107](0107-ponder-optimum-bonus.md) | optimumの1.25倍 | 棄却 |

いずれも参照実装の前提となる4つの仕組みのうち、1つずつを移したものだった。4本目が停止の予約で、
...
4. `optimum` が1.25倍されているので予算そのものが多い
...
探索を出力なしの中断し実時間で再起動する設計だった。**これを捨てて継続する形へ
変える**。

### 依存の棚卸し

実装の前に洗い出した。指示書の挙げた3点がすべて不足していた。
...
同じ形である。`Hit` の意味だけを「出力なしの中断」から「探索は続け、終わったら
出す」へ変えた。
...
最小思考時間を外した条件（`MinimumThinkingTime=1`・`RoundUpToFullSecond=false`）。
...
一時的な計測で追った。最小思考時間なし・ponder 400msの例である。
...
予約は起きなかった**。前提となる仕組みとして効いている。

最小思考時間ありでは `search_end = 1880 + 3008 = 4888` となり、`t2` 項がponderhitから
1880msを保証していた。
...
-54.3だったものが、前提となる4つの仕組みを揃えると正に転じる。

**1つずつでは負け、4つ揃えると勝つ**。一連の追従で最も明確に「群として
移す」ことの価値が出た例である。

### 条件の選定

条件の選定に難があった。**最小思考時間を外すと、前提となる仕組みの1つが消える**。
...
`minimum()` が0になり、**`set_search_end` の `t2` 項が働かなくなる**。実測では
...
打ち止めにする」と書いた。この判断は誤りだった。前提となる仕組みが1つ足りなかっただけである。
...
**測定条件の選定が難しい群だった**。最小思考時間を外すと前提となる仕組みの1つ（`t2` 項）が消え、
既定のままにすると時間切れになる。中間の300msを選んだが、これは
