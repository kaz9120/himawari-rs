...
LLR -0.48。効果なし〜ややマイナスと判断し打ち切り・棄却。
0054でqsearch TTが入った直後のベースは、履歴の質が変わっている可能性が
ある。improving項単体・history項単体の分解やしきい値の再設計は、再挑戦の
余地としてROADMAPの候補に残す。
- 関連ADR: [0028](0028-pruning-extensions.md), [0046](0046-correction-history.md), [0047](0047-continuation-history.md)

## Context

一連の探索改善の第9弾。現在のLMRはlog式テーブルに
PVで-1する調整しかない（`search.rs:770-780`）。手の筋の良し悪し
（history）や局面の改善傾向（improving）を無視して一律に
削っており、Stockfish系ではここに条件項を足すのがオーダリング系で
...
受け渡しが既存の値（improvingフラグ、オーダリングと同じ履歴合算）
だけで完結し、判定単位が明確。

### 案B: 案A + cutnode項（Stockfish完全形に近い形）

cutnode（期待カットノード）で r += 2 はStockfishで効果が大きいが、
ノード種別フラグを探索の再帰全体へ引数で渡す変更が要る。
案Aの成否を見てから別ADRで積む。
...
初期定数（チューニングしない）は `LMR_HIST_GOOD = 4000`、
`LMR_HIST_BAD = -4000` とする。履歴1本のクランプ値で、3本合算の
...
- リダクションの分散が広がる（最大でlog式+2）。fail-highの再探索
