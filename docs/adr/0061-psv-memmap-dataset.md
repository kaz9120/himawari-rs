# 0061: 教師データの事前シャッフルを廃止し、読み込みをRAM常駐＋forkに統一する
...
教師データを180M局面から370M局面へ増やしたところ、前処理とデータ
ローダの両方が詰まった。データは14.8GBで、開発機のRAMは48GBである。
...
`valid_385M.psv` の由来ファイルなので除外し、教師データと検証集合を
対局単位で分ける方針（P5で確立）を保つ。検証集合を据え置くことで、検証損失を
...
- RAM容量が教師データ量の上限を決める。48GB機では370M局面
