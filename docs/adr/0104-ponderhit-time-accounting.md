...
- ponderが当たった局面で1手に投じる総思考時間が減り、浮いた持ち時間が
  後続の手に回る
- ponderが外れた局面の挙動は変わらない
- Ponder無効時（`USI_Ponder=false`）は1.25倍が掛からず、挙動は現状のまま。
  SPRTの既定条件は影響を受けない
- 測定コストが上がる。並列4で回すため、同じ局数に倍の時間がかかる
- floodgateでponderを有効にしているかの確認が要る。無効なら実戦では
  効かない
- 再起動方式そのものは残る。ponderで進めた反復深化の途中経過は捨てたまま
  である。案Cを別ADRで検討する
