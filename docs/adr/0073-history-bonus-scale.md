...
この疎さではその前提が成り立たない。1回の更新量が35では値が十分に育たず、
...
このADRの変更は単体で棋力を上げるとは限らない。オーダリングの質が
...
平衡点16384に飽和した。値が上限付近へ寄るとオーダリングの解像度が落ちる。
...
オーダリングの挙動が全面的に変わる。[ADR-0047](0047-continuation-history.md)や
