...
[ADR-0033](0033-ponder.md)は、ponderhitで探索を出力なしで中断して実時間で
...
理由は時間の数え方そのものではなく、**再起動が反復深化の途中経過を捨てること**に
...
再起動が捨てているのはTTに入らないものである。反復深化のイテレーション、
...
置換表の当たりを増やす方向。ただし反復深化のイテレーションそのものは復元できない。
深さ1から読み直す手間は残る。
...
ponder時間の計上の再挑戦は、本ADRが入ったあとに別途行う。そのときは前提が揃っている。
...
同じ予算で深さが1深く、ノード数は1.7倍になった。ponderで積んだ8秒が
そのまま活きている。
...
深さは0.7稼ぐが、**1手あたり1.9倍の持ち時間を使う**。maximum
（615ms）も超えている。ponderhit時点で探索は深いところにおり、次の
イテレーションが予算より大きいためである。再起動なら深さ1から積み直す
ので、イテレーション境界がoptimumの近くに来る。

計時の起点をponderhitへ置いたままにしたことが原因である。エンジンは
「まだ何も使っていない」と思い、深い局面から新しいイテレーションを
始めてしまう。

### ponder時間の計上と組にしても伸びない

[ADR-0104](0104-ponderhit-time-accounting.md)の時間の数え方（起点を
...
|---|---|---|
| 再起動（変更前） | 226ms | 17.9 |
| 継続＋計上 | **199ms**（-12%） | **17.4**（-0.5） |

時間は12%浮くが、1手の深さが0.5落ちる。指し手は8手すべて一致した。
...
- **ponderまわりの時間管理は打ち止めにする**。3通り測ってすべて中立
  以下だった。計上だけ（[ADR-0104](0104-ponderhit-time-accounting.md)）が
