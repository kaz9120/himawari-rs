"""長く走るコマンドへの状態ファイルの配線（ADR-0220の2段目）を検証する。

状態ファイルの形式そのものは test_status.py が見る。ここでは、各コマンドの出力や
進み具合が状態ファイルへ写ることと、抜け方で終わりの状態が決まることを見る。
...
def exp_runner(tmp_path, monkeypatch):
    ...
    def fake_run(argv, **_):
        # ステップを始める前に、状態ファイルが今のステップを指している
