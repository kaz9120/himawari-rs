...
--from を付けると、その評価ファイルを各構成へゼロ埋めで広げる。広げた
ぶんの重みはゼロなので**評価値が元と完全に一致し、構成を変えても探索木が
変わらない**。速度の差だけを取り出せる。付けないと構成ごとに乱数ネットを
作るが、評価値が構成ごとに違うため探索木が揃わず、NPSを比べられない。

元ネットより小さい次元を持つ構成は指定できない（広げる向きにだけ使える）。
USAGE
}
...
for spec in "$@"; do
	...
	log_info "評価ファイル: ${net}"
	if [[ -n "$FROM" ]]; then
		"${target}/release/makenet" --expand "$FROM" --out "$net" >/dev/null
	else
