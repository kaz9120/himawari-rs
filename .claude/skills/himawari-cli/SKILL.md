...
hmwr --help              全体
hmwr <領域>              その領域のヘルプ
hmwr status              キュー・実行中・状態ファイル・直近の結果・資源・設定を1画面で出す
...
`--stop pairs:500` の実行を `pairs:1000` で続きから積める。記録のない
...
完了後は結果の定数を `crates/engine/src/tunables.rs` へ反映し、
...
**2本以上を並べて交互に測る**。1本ずつ別々に測った値を比べない。マシンの温度や
...
hmwr build shapes 256x16 512x16x32  構成ごとにエンジンとネットを対で作る
```

棋力が変わる変更をキューで測るときは、候補のブランチをpushし、specの
`build pair` に**コミットのSHA**を渡す。キューのworktreeは常にorigin/mainで、
...
**検証集合は学習データと同じ条件に揃える**（ADR-0136）。静止化した教師で
学習するなら検証集合も静止化する。条件がずれると最良チェックポイントの選択が
歪む。

**検証損失を足切りに使わない**（ADR-0158）。初期値の系列が違うだけで0.00136
動く。**採否は対局で決める。**

`net probe` は表現の測定で、ネットを作らない。FTを凍結してヒートマップを当てる
後段だけを学習し、上位5マスの的中率を検証行へ出す。**ベースラインと並べて読む**。
頻度ベースラインは同じ行に並び、乱数ベースライン（乱数初期値のFT）は `--init-net none` で別に測る。
...
```
hmwr exp check                 worktreeのspecを検査する（CIと同じ）
...
書き写さない。対局は結果ファイル、学習は学習の記録、データは完了マーカーから読むので、
転記の誤りが構造的に起きない。

止まったら同じコマンドで続きから走る。完了したステップは飛ばし、学習と対局は
それぞれの再開の仕組みで続く。完了したステップのコマンドをspecで書き換えると
止まる。手順を変えるなら、別の名前のspecにする。

### 実験をキューで無人実行する

```
hmwr queue status      キューと一時停止の状態
hmwr status            キューに加え、実行中のステップ・状態ファイル・直近の結果・資源も1画面で
...
`diag` は1本のADRのために作った測定の置き場で、互換を保証するインターフェースではない。
...
名前を変えるか `--force` で作り直す。完了マーカーのない古い出力も同じ扱いになる。

`data rm` が消すのは完了マーカーのある出力だけである。完了マーカーに作り方が残っているので、
...
手を持たないので使えない。出力は202バイト固定長で、元のpsvの40バイトにヒートマップの
...
```
hmwr clean            消せるものの一覧と合計を出す（dry-run）
