...
構成値のように変わり続ける事実はここへ書かず、一次情報の置き場（Releases・
...
[docs/ROADMAP.md](docs/ROADMAP.md)の「現在地」から辿れる。
...
全オプションと値域は、エンジンへ `usi` と入力したときの出力が一次情報になる。

### 最適なスレッド数を測る

`Threads` の最適値は機械ごとに違う。コア数だけでは決まらず、混成コアや
電力の上限で少ないほうを選ぶべき場合もある。エンジン自身に測るモードを
持たせてある。

```
himawari threadtune --eval <評価関数> --current 6 --hours 8
```

最大スレッド数でウォームアップしてから候補ごとの持続NPSを測り、伸びない候補を
落とす。次に現在の設定（`--current`）を起点に隣の候補と自己対局し、
勝った側へ進む。差を区別できないときは少ないほうを採る。最後に
`推奨: Threads=N` を1行で出す。放流と同じ電源モードとAC接続で測る。
一晩（`--hours 8`）を勧める。`--nps-only` はNPSの表だけを出して終わる。

候補の上限は既定で論理コア−2にしてある。実戦では指し手を中継する
クライアントが別プロセスで動くので、全論理コアを埋めると中継が遅れて
切れ負けする。このツールは自分が中継役なのでその遅れを見られない。
...
日付はUTCで決まるので、行の先頭に出るUNIX時刻と基準が揃う。常時取るときは
これを入れて、古い日のファイルを消す。
...
ここに無い問題は[Issues](../../issues/new/choose)の「不具合の報告」から
知らせてほしい。版・環境・起きたことを書く欄がある。提案は「提案・要望」へ。
...
**バイナリとネットは対で使う**。次元やファイル形式の版が食い違うと
読み込みで落ちる。既定のビルドへ渡すネットは `hmwr/config.py` の
`EVAL_FILE` にある（計測・対局が実際に使う値なので、常に最新に保たれる）。
...
hmwr status                         今の状態（キュー・実行中・直近の結果・資源・設定）
...
状況・設計・手順はリポジトリ内の文書で管理する。GitHub Issuesは、細かい改善（`micro` ラベル）と
実験キューへの登録に使う。
